class DealWithItApp {
  constructor() {
    ...
    this.isProcessing = false // Track processing state
  ...
  initializeEventListeners() {
    ...
    uploadFileBtn?.addEventListener('click', () => {
      if (this.isProcessing) return // Prevent tab switching during processing
  ...
  updateTabDisplay() {
    ...
    // Add visual indication if processing (disable styling)
    const baseClass =
      'px-4 py-2 text-sm font-semibold rounded-lg w-full text-center'
    const disabledClass = this.isProcessing
      ? ' opacity-50 cursor-not-allowed'
      : ''

    if (this.activeTab === 'file') {
      // Active file tab
      uploadFileBtn.className =
        baseClass + ' bg-white shadow text-gray-800' + disabledClass
      uploadFileBtn.setAttribute('aria-selected', 'true')

      // Inactive URL tab
      uploadUrlBtn.className =
        baseClass + ' text-gray-600 hover:text-gray-800' + disabledClass
  ...
  resetForNewImage() {
    ...
    actionButtons.classList.add('hidden') // Hide copy/download buttons when switching tabs
    imageInput.value = ''
    imageUrlInput.value = ''
  ...
  async processImage() {
    ...
    const processingOverlay = document.getElementById('processingOverlay') // Declare once at function scope
